...
class FFSProvidersController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = HealthCareProvider::query()
            ...
            ->withCount(['claims as ffs_pending_count' => fn($q) =>
                $q->where('status', 'approved')->whereNull('payment_batch_id')
            ])
    ...
    public function spendTrend(Request $request)
    {
        ...
        // Get 12 months of completed FFS batch totals
        $ffsMonths = DB::table('payment_batches as b')
            ...
            ->where('b.completed_at', '>=', now()->subMonths(12)->startOfMonth())
            ->select([
                DB::raw("DATE_FORMAT(b.completed_at, '%b %Y') as period"),
                DB::raw("DATE_FORMAT(b.completed_at, '%Y-%m') as sort_key"),
                DB::raw('SUM(bi.amount) as ffs_paid'),
            ])
    ...
    public function createBatch(Request $request)
    {
        ...
        // Safety: ensure HCP is actually FFS or Hybrid
        ...
        $batch = PaymentBatch::create([
            'batch_number'  => PaymentBatch::generateBatchNumber(),
        ...
        $claims->each(fn($c) => $c->update(['payment_batch_id' => $batch->id]));

        return response()->json([
            'message' => "Batch created with {$claims->count()} FFS claims.",
            'data'    => $batch->load('claims'),
        ], 201);
    }
}
