/**
 * FILE: resources/js/pages/HelpCentrePage.jsx
 *
 * FIX: Made article data extraction explicit and unambiguous.
 *
 * API response structure (axios wraps HTTP body in .data):
 *   resp.data             = HTTP body  = { data: paginator, categories, featured, meta }
 *   resp.data.data        = paginator  = { current_page, data: [...items], total, ... }
 *   resp.data.data.data   = items array  ← articles live here
 *   resp.data.categories  = sidebar categories
 *   resp.data.meta        = { total, current_page, last_page }
 *
 * The old code used data?.data?.data?.data which is correct but hard to
 * debug when wrong. Now extracted via a named helper function.
 */
import React, { useState, useRef, useCallback } from 'react';
import { useNavigate, useParams, useSearchParams } from 'react-router-dom';
import { useQuery } from '@tanstack/react-query';
import {
    Search, BookOpen, ChevronRight, ThumbsUp, ThumbsDown,
    Edit, ArrowLeft, Star, X as XIcon,
} from 'lucide-react';
import apiClient from '../../api/client';
import ReactMarkdown from 'react-markdown';
import { useAuth } from '../../contexts/AuthContext';
import { LoadingSpinner } from '../../components/ui/index';
import { toast } from 'react-toastify';

// ── Data extraction helpers ────────────────────────────────────────────────
// Named so the shape is obvious rather than buried in optional chains.
//
// axios response   → resp
// HTTP body        → resp.data                  { data: paginator, categories, ... }
// paginator        → resp.data.data             { current_page, data:[items], total, ... }
// items array      → resp.data.data.data        ← articles

function getArticles(resp) { return resp?.data?.data?.data ?? []; }
function getCategories(resp) { return resp?.data?.categories ?? []; }
function getFeatured(resp) { return resp?.data?.featured ?? []; }
function getMeta(resp) {
    // Prefer explicit meta block; fall back to paginator-level fields.
    return resp?.data?.meta ?? {
        total: resp?.data?.data?.total ?? 0,
        current_page: resp?.data?.data?.current_page ?? 1,
        last_page: resp?.data?.data?.last_page ?? 1,
    };
}

// ── Root component ─────────────────────────────────────────────────────────
export default function HelpCentrePage() {
    const { slug } = useParams();
    return slug ? <ArticleView slug={slug} /> : <ArticleList />;
}

// ── Article list (home) ────────────────────────────────────────────────────
function ArticleList() {
    const navigate = useNavigate();
    const [searchParams, setSearchParams] = useSearchParams();
    const [query, setQuery] = useState(searchParams.get('q') ?? '');
    const [debouncedQ, setDebouncedQ] = useState(searchParams.get('q') ?? '');
    const [cat, setCat] = useState(searchParams.get('cat') ?? '');
    const { hasPermission } = useAuth();
    const timer = useRef(null);

    // Debounce: fire API call 400ms after user stops typing
    const handleQueryChange = useCallback((val) => {
        setQuery(val);
        clearTimeout(timer.current);
        timer.current = setTimeout(() => {
            setDebouncedQ(val);
            setSearchParams(val ? { q: val, ...(cat ? { cat } : {}) } : (cat ? { cat } : {}));
        }, 400);
    }, [cat, setSearchParams]);

    const clearSearch = () => {
        setQuery('');
        setDebouncedQ('');
        setSearchParams(cat ? { cat } : {});
    };

    const selectCategory = (key) => {
        setCat(key);
        setSearchParams(key
            ? { cat: key, ...(debouncedQ ? { q: debouncedQ } : {}) }
            : (debouncedQ ? { q: debouncedQ } : {})
        );
    };

    const { data: resp, isLoading, isError } = useQuery({
        queryKey: ['help-articles', debouncedQ, cat],
        queryFn: () => apiClient.get('/help', {
            params: {
                q: debouncedQ || undefined,
                category: cat || undefined,
            },
        }),
        staleTime: 60_000,
        retry: 1,
    });

    // Log the response to see its structure
    console.log('API Response:', resp);

    // For non-paginated response (using get())
    const articles = resp?.data?.data ?? [];  // Just one level!
    const categories = resp?.data?.categories ?? [];
    const featured = resp?.data?.featured ?? [];
    const meta = getMeta(resp);

    const isFiltered = debouncedQ || cat;

    return (
        <div>
            {/* ── Hero / search bar ──────────────────────────────────────── */}
            <div className="text-center py-4 px-4 mb-4 rounded-3"
                style={{ background: ' #1e3a5f', color: '#fff' }}>
                <BookOpen size={30} className="mb-3 opacity-75" />
                <h2 className="fw-bold mb-2">Help Centre</h2>
                <p className="mb-4 opacity-75" style={{ fontSize: 15 }}>
                    Find answers about enrollees, claims, reports, and everything in between.
                </p>
                <div className="row justify-content-center">
                    <div className="col-md-6">
                        <div className="input-group">
                            <span className="input-group-text bg-white border-end-0">
                                <Search size={16} style={{ color: '#64748b' }} />
                            </span>
                            <input
                                type="text"
                                className="form-control border-start-0"
                                placeholder="Search articles…"
                                value={query}
                                onChange={e => handleQueryChange(e.target.value)}
                                style={{ fontSize: 15 }}
                            />
                            {query && (
                                <button className="btn btn-light border" onClick={clearSearch}>
                                    <XIcon size={14} />
                                </button>
                            )}
                        </div>
                        {debouncedQ && (
                            <p className="mt-2 mb-0 opacity-75" style={{ fontSize: 13 }}>
                                Searching for <strong>"{debouncedQ}"</strong>
                                {cat && <> in <strong>{categories.find(c => c.key === cat)?.label}</strong></>}
                            </p>
                        )}
                    </div>
                </div>
            </div>

            <div className="row">
                {/* ── Sidebar ─────────────────────────────────────────────── */}
                <div className="col-md-3 mb-4">
                    {hasPermission?.('help.admin') && (
                        <button className="btn btn-outline-secondary btn-sm w-100 mb-3"
                            onClick={() => navigate('/help/admin')}>
                            <Edit size={13} className="me-1" />Manage Articles
                        </button>
                    )}
                    <div className="card border-0 shadow-sm">
                        <div className="card-header bg-white fw-semibold" style={{ fontSize: 13 }}>
                            Browse by Category
                        </div>
                        <div className="list-group list-group-flush">
                            <button
                                className={`list-group-item list-group-item-action d-flex justify-content-between align-items-center ${!cat ? 'active' : ''}`}
                                style={{ fontSize: 13, background: '#1e3a5f', color: '#ffffff' }}
                                onClick={() => selectCategory('')}
                            >
                                <span>All Articles</span>
                                <span className={`badge rounded-pill ${!cat ? 'bg-white text-primary' : 'bg-secondary'}`}>
                                    {meta.total || ''}
                                </span>
                            </button>
                            {categories.map(c => (
                                <button
                                    key={c.key}
                                    className={`list-group-item list-group-item-action d-flex justify-content-between align-items-center ${cat === c.key ? 'active' : ''}`}
                                    style={{ fontSize: 13 }}
                                    onClick={() => selectCategory(c.key)}
                                >
                                    <span>{c.icon} {c.label}</span>
                                    <span className={`badge rounded-pill ${cat === c.key ? 'bg-white text-primary' : 'bg-secondary'}`}>
                                        {c.count}
                                    </span>
                                </button>
                            ))}
                        </div>
                    </div>
                </div>

                {/* ── Main content ─────────────────────────────────────────── */}
                <div className="col-md-9">

                    {/* Featured grid — only when no filter active */}
                    {!isFiltered && featured.length > 0 && (
                        <div className="mb-4">
                            <h6 className="fw-semibold mb-3 d-flex align-items-center gap-2">
                                <Star size={15} style={{ color: '#f59e0b' }} />
                                Start Here
                            </h6>
                            <div className="row g-3 mb-4">
                                {featured.map(a => (
                                    <div key={a.id} className="col-md-6">
                                        <div
                                            className="card border-0 shadow-sm h-100"
                                            style={{ cursor: 'pointer', transition: 'box-shadow .15s' }}
                                            onMouseEnter={e => e.currentTarget.style.boxShadow = '0 4px 20px rgba(0,0,0,.1)'}
                                            onMouseLeave={e => e.currentTarget.style.boxShadow = ''}
                                            onClick={() => navigate(`/help/${a.slug}`)}
                                        >
                                            <div className="card-body py-3">
                                                <div className="d-flex align-items-start gap-2">
                                                    <span style={{ fontSize: 24 }}>{a.category_icon}</span>
                                                    <div>
                                                        <div className="fw-semibold" style={{ fontSize: 13 }}>{a.title}</div>
                                                        <div className="text-muted" style={{ fontSize: 11 }}>{a.category_label}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ))}
                            </div>
                            <hr className="mb-3" />
                            <p className="text-muted mb-3" style={{ fontSize: 13 }}>
                                All articles ({meta.total}):
                            </p>
                        </div>
                    )}

                    {/* Active category pill */}
                    {cat && (
                        <div className="mb-3 d-flex align-items-center gap-2">
                            <span style={{ fontSize: 13, color: '#64748b' }}>Category:</span>
                            <span className="badge bg-primary" style={{ fontSize: 12 }}>
                                {categories.find(c => c.key === cat)?.icon}{' '}
                                {categories.find(c => c.key === cat)?.label ?? cat}
                            </span>
                            <button className="btn btn-sm btn-link p-0 text-muted"
                                onClick={() => selectCategory('')}>
                                <XIcon size={12} /> Clear
                            </button>
                        </div>
                    )}

                    {/* Article list */}
                    {isLoading ? (
                        <div className="py-5 text-center"><LoadingSpinner /></div>
                    ) : isError ? (
                        <div className="alert alert-danger" style={{ fontSize: 13 }}>
                            Failed to load articles. Please refresh the page.
                        </div>
                    ) : articles.length === 0 ? (
                        <div className="text-center py-5 text-muted">
                            <Search size={40} className="mb-3 opacity-25" />
                            {debouncedQ
                                ? <p>No articles found for <strong>"{debouncedQ}"</strong>.</p>
                                : cat
                                    ? <p>No articles in this category yet.</p>
                                    : <p>No articles available.</p>
                            }
                            {isFiltered && (
                                <button className="btn btn-sm btn-outline-secondary mt-1"
                                    onClick={() => { clearSearch(); selectCategory(''); }}>
                                    Show all articles
                                </button>
                            )}
                        </div>
                    ) : (
                        <div className="card border-0 shadow-sm">
                            {articles.map((a, i) => (
                                <div
                                    key={a.id}
                                    className={`d-flex align-items-start gap-3 p-3 ${i < articles.length - 1 ? 'border-bottom' : ''}`}
                                    style={{ cursor: 'pointer', transition: 'background .1s' }}
                                    onMouseEnter={e => e.currentTarget.style.background = '#f8fafc'}
                                    onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
                                    onClick={() => navigate(`/help/${a.slug}`)}
                                >
                                    <span style={{ fontSize: 22, lineHeight: 1, marginTop: 3, flexShrink: 0 }}>
                                        {a.category_icon}
                                    </span>
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                        <div className="fw-semibold" style={{ fontSize: 14 }}>{a.title}</div>
                                        <div className="text-muted" style={{ fontSize: 12 }}>{a.category_label}</div>
                                        {a.excerpt && (
                                            <div className="text-muted mt-1" style={{
                                                fontSize: 12,
                                                overflow: 'hidden', textOverflow: 'ellipsis',
                                                display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical',
                                            }}>
                                                {a.excerpt}
                                            </div>
                                        )}
                                    </div>
                                    <ChevronRight size={16} style={{ color: '#94a3b8', marginTop: 4, flexShrink: 0 }} />
                                </div>
                            ))}
                        </div>
                    )}
                </div>
            </div>
        </div>
    );
}

// ── Single article view ────────────────────────────────────────────────────
function ArticleView({ slug }) {
    const navigate = useNavigate();
    const [feedback, setFeedback] = useState(false);
    const { hasPermission } = useAuth();

    const { data: resp, isLoading } = useQuery({
        queryKey: ['help-article-full', slug],
        queryFn: () => apiClient.get(`/help/${slug}`),
        staleTime: 60_000,
    });

    // show() returns { data: article, related: [] } (not paginated)
    const article = resp?.data?.data;
    const related = resp?.data?.related ?? [];

    const sendFeedback = async (helpful) => {
        await apiClient.post(`/help/${article.id}/feedback`, { helpful });
        setFeedback(true);
        toast.success('Thanks for your feedback!');
    };

    if (isLoading) return <div className="py-5 text-center"><LoadingSpinner /></div>;
    if (!article) return (
        <div className="text-center py-5 text-muted">
            Article not found.{' '}
            <button className="btn btn-sm btn-link p-0" onClick={() => navigate('/help')}>
                Back to Help Centre
            </button>
        </div>
    );

    return (
        <div className="row justify-content-center">
            <div className="col-lg-8">
                <nav className="mb-3" style={{ fontSize: 13 }}>
                    <button className="btn btn-sm btn-link p-0 text-muted" onClick={() => navigate('/help')}>
                        <ArrowLeft size={13} className="me-1" />Help Centre
                    </button>
                    <span className="text-muted mx-2">/</span>
                    <span className="text-muted">{article.category_label}</span>
                </nav>

                <div className="card border-0 shadow-sm">
                    <div className="card-body p-4 p-md-5">
                        <div className="d-flex align-items-start justify-content-between mb-3">
                            <span className="badge" style={{ background: '#e8f0fe', color: '#1d4ed8', fontSize: 12 }}>
                                {article.category_icon} {article.category_label}
                            </span>
                            {hasPermission?.('help.admin') && (
                                <button className="btn btn-sm btn-outline-secondary"
                                    onClick={() => navigate(`/help/admin/edit/${article.id}`)}>
                                    <Edit size={13} className="me-1" />Edit
                                </button>
                            )}
                        </div>

                        <div className="markdown-body" style={{ fontSize: 14, lineHeight: 1.8 }}>
                            <ReactMarkdown
                                components={{
                                    h1: ({ node, ...p }) => <h2 className="fw-bold mt-0 mb-3" style={{ fontSize: 22 }} {...p} />,
                                    h2: ({ node, ...p }) => <h3 className="fw-semibold mt-4 mb-2" style={{ fontSize: 17 }} {...p} />,
                                    h3: ({ node, ...p }) => <h4 className="fw-semibold mt-3 mb-2" style={{ fontSize: 15 }} {...p} />,
                                    table: ({ node, ...p }) => (
                                        <div className="table-responsive mb-3">
                                            <table className="table table-bordered table-sm" {...p} />
                                        </div>
                                    ),
                                    thead: ({ node, ...p }) => <thead className="table-light" {...p} />,
                                    blockquote: ({ node, ...p }) => (
                                        <blockquote className="border-start border-primary ps-3 text-muted"
                                            style={{ fontSize: 13 }} {...p} />
                                    ),
                                    code: ({ node, inline, ...p }) => inline
                                        ? <code className="bg-light px-1 rounded" style={{ fontSize: 12 }} {...p} />
                                        : <pre className="bg-light p-3 rounded" style={{ fontSize: 12, overflowX: 'auto' }}><code {...p} /></pre>,
                                }}
                            >
                                {article.content}
                            </ReactMarkdown>
                        </div>

                        <p className="text-muted mt-4 mb-0" style={{ fontSize: 11 }}>
                            Last updated: {new Date(article.updated_at).toLocaleDateString('en-NG', {
                                day: 'numeric', month: 'long', year: 'numeric',
                            })}
                        </p>

                        <div className="border-top mt-4 pt-3">
                            {feedback ? (
                                <p className="text-success text-center mb-0" style={{ fontSize: 13 }}>
                                    ✓ Thanks for your feedback!
                                </p>
                            ) : (
                                <div className="d-flex align-items-center gap-3 justify-content-center">
                                    <span className="text-muted" style={{ fontSize: 13 }}>Was this helpful?</span>
                                    <button className="btn btn-sm btn-outline-success" onClick={() => sendFeedback(true)}>
                                        <ThumbsUp size={13} className="me-1" />Yes
                                    </button>
                                    <button className="btn btn-sm btn-outline-secondary" onClick={() => sendFeedback(false)}>
                                        <ThumbsDown size={13} className="me-1" />Not really
                                    </button>
                                </div>
                            )}
                        </div>
                    </div>
                </div>

                {related.length > 0 && (
                    <div className="mt-4">
                        <h6 className="fw-semibold mb-3">Related Articles</h6>
                        <div className="card border-0 shadow-sm">
                            {related.map((r, i) => (
                                <button
                                    key={r.id}
                                    className={`w-100 text-start px-4 py-3 d-flex align-items-center gap-2 bg-transparent border-0 ${i < related.length - 1 ? 'border-bottom' : ''}`}
                                    onClick={() => navigate(`/help/${r.slug}`)}
                                    style={{ fontSize: 13, transition: 'background .1s' }}
                                    onMouseEnter={e => e.currentTarget.style.background = '#f8fafc'}
                                    onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
                                >
                                    <ChevronRight size={14} style={{ color: '#94a3b8' }} />
                                    {r.title}
                                </button>
                            ))}
                        </div>
                    </div>
                )}

                <div className="text-center mt-4">
                    <button className="btn btn-sm btn-outline-secondary" onClick={() => navigate('/help')}>
                        <ArrowLeft size={13} className="me-1" />Back to Help Centre
                    </button>
                </div>
            </div>
        </div>
    );
}