/**
 * FILE: resources/js/pages/plans/PlanListPage.jsx
 *
 * Shown as a tab inside CorporateDetailPage.
 * Also accessible standalone at /corporates/:corporateId/plans
 *
 * Shows all plans for a corporate with KPIs, tier badges,
 * enrollee counts, and quick-action buttons.
 */
import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
import { toast } from 'react-toastify';
import { Plus, Copy, XCircle, Users, ShieldCheck, FileText, TrendingUp } from 'lucide-react';
import { fetchPlans, discontinuePlan, duplicatePlan, fetchAllPlans } from '../../api/index';
import { StatusBadge, LoadingSpinner, ErrorAlert, EmptyState } from '../../components/ui/index';
...
export default function PlanListPage({ corporateId, corporateName }) {
    ...
    const { data, isLoading, error, refetch } = useQuery({
        queryKey: ['plans', corporateId],
        queryFn:  () => fetchPlans(corporateId),
        enabled:  authReady && !!corporateId,
        staleTime: 60_000,
    });
    ...
    const discontinueMutation = useMutation({
        mutationFn: (planId) => discontinuePlan(corporateId, planId),
        onSuccess: () => {
            toast.success('Plan discontinued.');
            qc.invalidateQueries({ queryKey: ['plans', corporateId] });
        },
    ...
    const duplicateMutation = useMutation({
        mutationFn: ({ planId, name }) => duplicatePlan(corporateId, planId, { plan_name: name }),
        onSuccess: (res) => {
    ...
    return (
        <div>
            ...
            <div className="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h5 className="mb-0 fw-semibold">Health Plans</h5>
                    <small className="text-muted">
                        {activePlans.length} active plan{activePlans.length !== 1 ? 's' : ''} · {totalEnrollees.toLocaleString()} enrollees
                    </small>
                </div>
                {hasPermission('plans.create') && (
                    <button
                        className="btn btn-primary btn-sm d-flex align-items-center gap-2"
                        onClick={() => navigate(`/corporates/${corporateId}/plans/new`)}
                    >
            ...
            {isLoading ? (
                ...
                <EmptyState
                    ...
                    title="No plans yet"
                    description="Create a health plan to start enrolling members."
                    action={
                        hasPermission('plans.create') && (
                            <button
                        ...
                        return (
                            <div key={plan.id} className="col-md-6 col-lg-4">
                                <div
                                    ...
                                    onClick={() => navigate(`/corporates/${corporateId}/plans/${plan.id}`)}
                                >
                                    {/* Card header stripe */}
                                        ...
                                        {hasPermission('plans.edit') && plan.status !== 'discontinued' && (
                                            <div className="d-flex gap-2 pt-2 border-top">
                                                <button
                                                    ...
                                                    onClick={e => {
                                                        e.stopPropagation();
                                                        navigate(`/corporates/${corporateId}/plans/${plan.id}/edit`);
                                                    }}
                                                >
                                                    Edit
                                                </button>
