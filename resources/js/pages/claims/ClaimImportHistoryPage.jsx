...
import { PageHeader, LoadingSpinner, ErrorAlert, Pagination, StatusBadge } from '../../components/ui/index';
...
export default function ClaimImportHistoryPage() {
    ...
    const { data, isLoading, error, refetch } = useQuery({
        ...
        queryFn: () => fetchImportBatches({ page, status: statusFilter || undefined }),
    });
            ...
            <div className="card border-0 shadow-sm mb-3">
                <div className="card-body py-2">
                    <div className="d-flex gap-2 align-items-center flex-wrap">
                        ...
                        {['', 'pending', 'mapped', 'validated', 'completed', 'failed'].map(s => (
                            <button key={s}
                            ...
                            <table className="table table-hover align-middle mb-0">
                                ...
                                <tbody>
                                    {batches.map(b => {
                                        ...
                                        return (
                                            <tr key={b.id}>
                                                <td className="ps-3 font-monospace">{b.batch_number}</td>
                                                <td>{b.hcp?.name || '—'}</td>
                                                <td>{b.claim_period}</td>
                                                <td style={{ fontSize: 12 }}>
                                                    {formatDateTime(b.created_at)}<br />
                                                    <small className="text-muted">by {b.uploaded_by?.name}</small>
                                                </td>
                                                <td className="text-end">{b.total_rows}</td>
                                                <td className="text-end text-success">{b.valid_rows}</td>
                                                <td className="text-end text-danger">{b.error_rows}</td>
                                                <td>
                                                    <span className="badge" style={{ background: style.bg, color: style.color, fontSize: 11 }}>
                                                        {style.label}
                                                    </span>
                                                </td>
                                                <td>
                                                    <button
                                                        ...
                                                        onClick={() => navigate(`/claims/import/${b.id}`)}
                                                    >
                                                        <Eye size={14} /> View
                                                    </button>
                                                </td>
