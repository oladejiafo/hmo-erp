...
import {
    ...
    TrendingUp, FileText, Download, Phone, Mail, MapPin, Building2,
} from 'lucide-react';
import {
...
import {
    fetchHCP, accreditHCP, blacklistHCP,
    fetchTariffs, createTariff as addTariff, updateTariff,
    fetchContracts, createContract,
    fetchHCPPerformance,
} from '../../api/index';
import { PageHeader, StatusBadge, LoadingSpinner, ErrorAlert, ConfirmModal } from '../../components/ui/index';
...
export default function HCPDetailPage() {
    ...
    const blacklistM = useMutation({
        ...
        onSuccess: () => { toast.success('HCP blacklisted.'); setBM(false); inv(); },
        onError: (e) => toast.error(e.response?.data?.message ?? 'Failed.'),
    });
    ...
    const perfRows = (() => {
        ...
        if (perfData.data && Array.isArray(perfData.data)) return perfData.data;
        return [];
    })();
    ...
    // Safely handle tariffs data
    const tariffs = (() => {
        if (!tariffData) return [];
        if (Array.isArray(tariffData)) return tariffData;
        if (tariffData.data && Array.isArray(tariffData.data)) return tariffData.data;
        return [];
    })();

    // Safely handle contracts data
    const contracts = (() => {
        if (!contractData) return [];
        if (Array.isArray(contractData)) return contractData;
        if (contractData.data && Array.isArray(contractData.data)) return contractData.data;
        return [];
    })();
        ...
        <div>
            ...
            <div className="d-flex align-items-start gap-3 mb-4">
                ...
                <div className="d-flex gap-2 flex-shrink-0">
                    ...
                    {canAccredit && hcp.status === 'pending' && (
            ...
            {tab === 'overview' && (
                <div className="row g-4">
                    <div className="col-md-6">
                        <div className="card border-0 shadow-sm h-100">
                            <div className="card-header bg-white border-0 pt-3 pb-0">
                                <h6 className="fw-semibold" style={{ fontSize: 13 }}><Building2 size={14} className="me-1 text-primary" />Provider Details</h6>
                            </div>
                    ...
                    <div className="col-md-6">
                        <div className="card border-0 shadow-sm h-100">
                            <div className="card-header bg-white border-0 pt-3 pb-0">
                                <h6 className="fw-semibold" style={{ fontSize: 13 }}><Phone size={14} className="me-1 text-primary" />Contact &amp; Banking</h6>
                            </div>
                            <div className="card-body">
                                <dl className="row mb-0" style={{ fontSize: 13 }}>
                                    {[
                            ...
                            contracts.map(c => (
                                <div key={c.id} className="card border-0 shadow-sm">
                                    <div className="card-body">
                                        ...
                                        {c.terms_summary && <p className="text-muted mb-0 mt-2" style={{ fontSize: 12 }}>{c.terms_summary}</p>}
                                        {c.document_path && (
            ...
            {tab === 'performance' && (
                <div className="row g-4">
                    <div className="col-12">
                        <div className="card border-0 shadow-sm">
                            <div className="card-header bg-white border-0 pt-3 pb-0">
                                <h6 className="fw-semibold" style={{ fontSize: 13 }}><TrendingUp size={14} className="me-1 text-primary" />Score Trend (last 6 months)</h6>
                            </div>
                            <div className="card-body">
                                {perfLoading ? (
                                    <div className="text-center py-4"><LoadingSpinner /></div>
                            ...
                            <div className="modal-content">
                                <div className="modal-header border-0">
                                    <h6 className="modal-title d-flex align-items-center gap-2">
                                        <AlertTriangle size={16} className="text-danger" /> Blacklist HCP
                                    </h6>
                                    <button className="btn-close" onClick={() => setBM(false)} />
                                </div>
                                <div className="modal-body">
                                    <p className="text-muted" style={{ fontSize: 13 }}>
                                        This will block {hcp.name} from submitting new claims. All pending claims will be flagged.
                                    </p>
                                    <label className="form-label fw-semibold" style={{ fontSize: 13 }}>Reason <span className="text-danger">*</span></label>
                                    <textarea className="form-control" rows={3} value={blacklistReason}
                                        onChange={e => setBR(e.target.value)} placeholder="Describe why this HCP is being blacklisted..." />
                                </div>
        ...
        <>
            ...
            <div className="modal d-block">
                <div className="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
                    <div className="modal-content">
                        <div className="modal-header"><h6 className="modal-title">{existing ? 'Edit' : 'Add'} Tariff</h6><button className="btn-close" onClick={onClose} /></div>
                        <div className="modal-body">
                            <div className="row g-3">
                                <div className="col-md-4">
                                    <label className="form-label fw-semibold" style={{ fontSize: 13 }}>Service Code *</label>
                                    <input className="form-control" value={f.service_code} onChange={e => setF(p => ({ ...p, service_code: e.target.value }))} placeholder="e.g. CONS-001" />
                                </div>
                                <div className="col-md-8">
                                    <label className="form-label fw-semibold" style={{ fontSize: 13 }}>Service Name *</label>
                                    <input className="form-control" value={f.service_name} onChange={e => setF(p => ({ ...p, service_name: e.target.value }))} />
                                </div>
                                <div className="col-md-4">
                                    <label className="form-label fw-semibold" style={{ fontSize: 13 }}>Category *</label>
                                    <select className="form-select" value={f.category} onChange={e => setF(p => ({ ...p, category: e.target.value }))}>
                                        {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
                                    </select>
                                </div>
                                <div className="col-md-4">
                                    <label className="form-label fw-semibold" style={{ fontSize: 13 }}>Agreed Price (₦) *</label>
                                    <input type="number" className="form-control" value={f.agreed_price} onChange={e => setF(p => ({ ...p, agreed_price: e.target.value }))} min="0" step="0.01" />
                                </div>
                                <div className="col-md-4">
                                    <label className="form-label fw-semibold" style={{ fontSize: 13 }}>NHIS Price (₦)</label>
                                    <input type="number" className="form-control" value={f.nhis_price} onChange={e => setF(p => ({ ...p, nhis_price: e.target.value }))} min="0" step="0.01" />
                                </div>
                                <div className="col-md-4">
                                    <label className="form-label fw-semibold" style={{ fontSize: 13 }}>Effective From *</label>
                                    <input type="date" className="form-control" value={f.effective_from} onChange={e => setF(p => ({ ...p, effective_from: e.target.value }))} />
                                </div>
                                <div className="col-md-4">
                                    <label className="form-label fw-semibold" style={{ fontSize: 13 }}>Effective To</label>
                                    <input type="date" className="form-control" value={f.effective_to} onChange={e => setF(p => ({ ...p, effective_to: e.target.value }))} />
                                </div>
                                <div className="col-md-4 d-flex align-items-end pb-2">
                                    <div className="form-check">
                                        <input type="checkbox" className="form-check-input" id="ta" checked={f.is_active} onChange={e => setF(p => ({ ...p, is_active: e.target.checked }))} />
                        ...
                        <div className="modal-footer">
                            ...
                            <button className="btn btn-primary" onClick={save} disabled={saving || !f.service_name || !f.agreed_price}>
...
function ContractModal({ hcpId, onClose, onSaved }) {
    const [f, setF] = useState({ contract_number: '', start_date: '', end_date: '', payment_model: 'fee_for_service', capitation_rate: '', terms_summary: '' });
    const [saving, setSaving] = useState(false);
    const save = async () => {
        try {
            setSaving(true);
        ...
        <>
            ...
            <div className="modal d-block">
                <div className="modal-dialog modal-lg modal-dialog-centered">
                    <div className="modal-content">
                        <div className="modal-header"><h6 className="modal-title">New Contract</h6><button className="btn-close" onClick={onClose} /></div>
                        <div className="modal-body">
                            <div className="row g-3">
                                <div className="col-md-6">
                                    <label className="form-label fw-semibold" style={{ fontSize: 13 }}>Contract Number *</label>
                                    <input className="form-control" value={f.contract_number} onChange={e => setF(p => ({ ...p, contract_number: e.target.value }))} />
                                </div>
                                <div className="col-md-3">
                                    <label className="form-label fw-semibold" style={{ fontSize: 13 }}>Start Date</label>
                                    <input type="date" className="form-control" value={f.start_date} onChange={e => setF(p => ({ ...p, start_date: e.target.value }))} />
                                </div>
                                <div className="col-md-3">
                                    <label className="form-label fw-semibold" style={{ fontSize: 13 }}>End Date</label>
                                    <input type="date" className="form-control" value={f.end_date} onChange={e => setF(p => ({ ...p, end_date: e.target.value }))} />
                                </div>
                                <div className="col-md-4">
                                    <label className="form-label fw-semibold" style={{ fontSize: 13 }}>Payment Model</label>
                                    <select className="form-select" value={f.payment_model} onChange={e => setF(p => ({ ...p, payment_model: e.target.value }))}>
                                ...
                                {['capitation', 'hybrid'].includes(f.payment_model) && (
                                    <div className="col-md-4">
                                        ...
                                        <input type="number" className="form-control" value={f.capitation_rate} onChange={e => setF(p => ({ ...p, capitation_rate: e.target.value }))} min="0" />
                                    </div>
                                )}
                                <div className="col-12">
                                    <label className="form-label fw-semibold" style={{ fontSize: 13 }}>Terms Summary</label>
                                    <textarea className="form-control" rows={3} value={f.terms_summary} onChange={e => setF(p => ({ ...p, terms_summary: e.target.value }))} />
                                </div>
                                <div className="col-12">
                                    <label className="form-label fw-semibold" style={{ fontSize: 13 }}>Contract Document (PDF)</label>
                                    <input type="file" className="form-control" accept=".pdf" onChange={e => setF(p => ({ ...p, document: e.target.files[0] }))} />
                                </div>
                            </div>
                        </div>
                        <div className="modal-footer">
                            <button className="btn btn-light" onClick={onClose}>Cancel</button>
                            <button className="btn btn-primary" onClick={save} disabled={saving || !f.contract_number}>
